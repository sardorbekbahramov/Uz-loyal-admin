...
const News = () => {
  ...
  const getData = () => {
    setLoading(true);
    axios
      .get(`https://api.dezinfeksiyatashkent.uz/api/news`, {
        headers: {
  ...
  const handleOk = (values) => {
    ...
    axios({
      ...
      .then((res) => {
        if (res && res.data) {
          message.success(
            form.id ? "Blogs updated successfully" : "Blogs added successfully"
          );
          handleCancel();
          getData();
        } else {
      ...
      images: [
        {
          ...
          name: "image",
          status: "done",
          url: `${urlimage}${item?.blog_images[0]?.image?.src}`,
        },
      ],
    });
    form.id = item.id;
  ...
  const dataSource = blogs.map((item, index) => {
    console.log(item.news_images[0]);
    const imageUrl =
      item.news_images && item.news_images.length > 0
        ? `${urlimage}${item.news_images[0]?.image?.src || ""}`
        : "";
    ...
    return {
      ...
      action: (
        <>
          ...
          <Button onClick={() => deleteUser(item.id)} type="primary" danger>
            Delete
          </Button>
        </>
      ),
  ...
  const deleteUser = (id) => {
    ...
    Modal.confirm({
      ...
      onOk() {
        axios
          .delete(`https://api.dezinfeksiyatashkent.uz/api/news/${id}`, config)
          .then((res) => {
